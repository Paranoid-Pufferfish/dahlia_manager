...
int main() {
    ...
    char TextBox003Text[128] = "Path/to/file";
    ...
    while (!WindowShouldClose()) // Detect window close button or ESC key
    {
        ...
        if (WindowBox000Active) {
            ...
            if (GuiTextBox((Rectangle){anchor01.x + 168, anchor01.y + 48, 240, 24}, TextBox003Text, 128,
                           TextBox003EditMode))
                TextBox003EditMode = !TextBox003EditMode;
            GuiLabel((Rectangle){anchor01.x + 72, anchor01.y + 48, 96, 24}, Label004Text);
            if (GuiValueBox((Rectangle){anchor01.x + 168, anchor01.y + 96, 72, 24}, nullptr, &ChibiWidthValue, 0, 100,
                            ChibiWidthEditMode))
                ChibiWidthEditMode = !ChibiWidthEditMode;
            GuiLabel((Rectangle){anchor01.x + 240, anchor01.y + 96, 12, 24}, Label006Text);
            if (GuiValueBox((Rectangle){anchor01.x + 248, anchor01.y + 96, 72, 24}, nullptr, &ChibiHeightValue, 0, 100,
                            ChibiHeightEditMode))
                ChibiHeightEditMode = !ChibiHeightEditMode;
            GuiLabel((Rectangle){anchor01.x + 72, anchor01.y + 96, 96, 24}, Label008Text);
            if (GuiValueBox((Rectangle){anchor01.x + 168, anchor01.y + 144, 72, 24}, nullptr, &XPosValue, 0, 100,
                            XPosEditMode))
                XPosEditMode = !XPosEditMode;
            if (GuiValueBox((Rectangle){anchor01.x + 248, anchor01.y + 144, 72, 24}, nullptr, &YPosValue, 0, 100,
                            YPosEditMode))
            ...
            if (GuiValueBox((Rectangle){anchor02.x + 312, anchor02.y + 72, 24, 24}, nullptr, &ChibiIndexValue, 0, 100,
                            ChibiIndexEditMode))
